    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100/50 pb-16 sm:pb-0 dark:from-gray-950 dark:to-gray-900 bg-pattern">
      ...
      <main className="mx-auto max-w-5xl px-4 py-5 sm:py-6 sm:ml-14 relative z-10">
        <div className="animate-page-enter" key={activeTab}>
          ...
          {activeTab === 'expenses' && (
            <Expenses showToast={showToast} showConfirm={showConfirm} dataVersion={dataVersion} />
          )}
          {activeTab === 'advances' && (
            <Advances showToast={showToast} showConfirm={showConfirm} dataVersion={dataVersion} />
          )}
          {activeTab === 'contributions' && (
            <Contributions showToast={showToast} showConfirm={showConfirm} dataVersion={dataVersion} />
          )}
...
function Dashboard({ showToast, dataVersion }) {
          ...
          <div className="space-y-2">
            {recentExpenses.map((exp, idx) => (
              ...
              >
                ...
                <span className="ml-3 text-sm font-bold text-gray-900 dark:text-gray-100">{formatAmount(exp.amount)}</span>
              </div>
            ))}
...
function Expenses({ showToast, showConfirm, dataVersion }) {
...
function Advances({ showToast, showConfirm, dataVersion }) {
  const [bulkMode, setBulkMode] = useState(false)
  const { formatAmount, getCurrencySymbol } = useSettings()
...
function Contributions({ showToast, showConfirm, dataVersion }) {
  const [bulkMode, setBulkMode] = useState(false)
  const { formatAmount, getCurrencySymbol } = useSettings()
...
function Settlement({ showToast, dataVersion }) {
  const { formatAmount } = useSettings()
  const [balances, setBalances] = useState([])
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="card !p-0 overflow-hidden dark:border-gray-700/80">
        <DesktopTable
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4 text-sm leading-relaxed text-gray-600 dark:text-gray-400">
          ...
          <div className="rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2.5 text-xs text-emerald-800 dark:border-emerald-800/50 dark:bg-emerald-950/30 dark:text-emerald-300">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4 text-sm leading-relaxed text-gray-600 dark:text-gray-400">
          ...
          <p className="flex items-center gap-2 rounded-xl bg-rose-50 px-3 py-2.5 text-xs text-rose-800 dark:bg-rose-950/30 dark:text-rose-300">
      ...
      <div className="space-y-4">
        {sections.map((section, idx) => {
          ...
          return (
            <div
              key={section.title}
              className="animate-slide-up card !p-0 overflow-hidden dark:border-gray-700/80"
              style={{ animationDelay: `${idx * 60}ms` }}
            >
              <div className="flex items-center gap-3 border-b border-gray-100 bg-gray-50/50 dark:bg-gray-800/30 px-4 py-3 sm:px-5">
...
function ErrorBox({ msg }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-red-100 bg-red-50 p-4 text-sm leading-relaxed text-red-700 dark:border-red-800/50 dark:bg-red-950/30 dark:text-red-300">
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-red-100 dark:bg-red-900/50">
        <AlertCircle className="h-4 w-4 text-red-600 dark:text-red-400" />
      </div>
...
function FormField({ label, error, children }) {
  return (
    <div>
      ...
      {children}
      {error && (
        <p className="mt-1.5 flex items-center gap-1 text-xs font-medium text-red-500 dark:text-red-400 animate-fade-in">
...
function DeleteBtn({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="btn-ghost rounded-lg p-2 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/40 dark:hover:text-red-400"
      title="Delete"
      aria-label="Delete"
    >
      <Trash2 className="h-4 w-4" />
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-white/85 p-3.5 shadow-sm ring-1 ring-gray-100 backdrop-blur-lg dark:bg-gray-900/40 dark:backdrop-blur-lg dark:ring-gray-700/40 transition-all hover:shadow-md active:scale-[0.99]">
      ...
      <div className="flex items-center gap-1">
        <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{formatAmount(amount)}</span>
    ...
    <div className="card !p-0 overflow-hidden dark:border-gray-700/80">
      ...
      <div className="hidden overflow-x-auto sm:block">
        <table className="w-full text-sm">
          ...
          {hasData && (
            <tfoot className="border-t-2 border-gray-100 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-800/30">
              <tr>
                <td colSpan={totalColSpan + (selectable ? 1 : 0)} className="px-4 py-3.5 text-sm font-semibold text-gray-700 dark:text-gray-300">Total</td>
                <td className="px-4 py-3.5 text-right text-sm font-bold text-gray-900 dark:text-gray-100">{formatAmount(total)}</td>
        ...
        {!hasData ? (
          ...
          rows.map((row) => (
            <div key={row.id} className={`flex items-center justify-between rounded-xl bg-white/85 p-3 shadow-sm ring-1 ring-gray-100 backdrop-blur-lg dark:bg-gray-900/40 dark:backdrop-blur-lg dark:ring-gray-700/40 ${selectable ? 'pl-2' : ''}`}>
              ...
              <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{formatAmount(row.amount || 0)}</div>
            </div>
          ))
        )}
        {hasData && (
